...
resource "aws_secretsmanager_secret_version" "tfe_tls_cert_secret" {
  ...
  secret_string = base64encode(tls_locally_signed_cert.internal_cluster_signed_cert.cert_pem)
}
...
resource "aws_secretsmanager_secret_version" "tfe_tls_privkey_secret" {
  ...
  secret_string = base64encode(tls_private_key.internal_cluster_private_key.private_key_pem)
}
...
resource "aws_secretsmanager_secret_version" "tfe_tls_ca_bundle_secret" {
  ...
  secret_string = base64encode("${tls_self_signed_cert.internal_ca_cert.cert_pem}${tls_self_signed_cert.ca_cert.cert_pem}")
}
